...
SELECT * FROM read_csv_auto('../raws/data/raw_customers.csv');

-- 店舗テーブルの作成とデータインポート
CREATE OR REPLACE TABLE jaffle_shop_raw.stores AS
SELECT * FROM read_csv_auto('../raws/data/raw_stores.csv');

-- 注文テーブルの作成とデータインポート
CREATE OR REPLACE TABLE jaffle_shop_raw.orders AS
SELECT * FROM read_csv_auto('../raws/data/raw_orders.csv');

-- 製品テーブルの作成とデータインポート
CREATE OR REPLACE TABLE jaffle_shop_raw.products AS
SELECT * FROM read_csv_auto('../raws/data/raw_products.csv');

-- アイテムテーブルの作成とデータインポート（注文と製品を紐付ける）
CREATE OR REPLACE TABLE jaffle_shop_raw.items AS
SELECT * FROM read_csv_auto('../raws/data/raw_items.csv');

-- サプライテーブルの作成とデータインポート（材料・資材）
CREATE OR REPLACE TABLE jaffle_shop_raw.supplies AS
SELECT * FROM read_csv_auto('../raws/data/raw_supplies.csv');
